...
//			});
	 ...
	 * @param beforeRect 옮겨진 후 절대좌표
	 */
	public void updateCoveringInteractionFragments(Rectangle beforeRect) {
		...
		Rectangle afterRect = this.getFigure().getBounds().getCopy();
		...
		if (afterRect.width == 0 && afterRect.height == 0) { // 새 Lifeline 생성하는 경우 Interaction 내 모든 CF을 check			
			cfEditPartsToCheck = ApexSequenceUtil.apexGetAllCombinedFragmentEditParts(this);			
		} else { // 새로 생성이 아닌 lifeline 경계 변경인 경우
			
			if (beforeRect.x != afterRect.x ) { // 좌우측으로 이동 시 lifeline의 이동 전후 위치를 포함하던 CF을 check

				List<CombinedFragmentEditPart> cfEnclosingBeforeRect = ApexSequenceUtil.apexGetPositionallyLifelineCoveringCFEditParts(afterRect, this);
				List<CombinedFragmentEditPart> cfEnclosingAfterRect = ApexSequenceUtil.apexGetPositionallyLifelineCoveringCFEditParts(beforeRect, this);
