...
public abstract class InteractionFragmentEditPart extends ShapeNodeEditPart {
	 ...
	 * @param newBound 절대좌표로 환산된 새 CF Bounds
...
//*/
		for(Object child : lifelineEditPartsToCheck) {
			...
			LifelineEditPart lifelineEditPart1 = (LifelineEditPart)child;
			Lifeline lifeline1 = (Lifeline)lifelineEditPart1.resolveSemanticElement();
			Rectangle lifelineRect = lifelineEditPart1.getFigure().getBounds().getCopy();
			lifelineEditPart1.getFigure().translateToAbsolute(lifelineRect);
			//lifelineEditPart.getFigure().getParent().translateToAbsolute(lifelineRect);
			
//*8
System.out.println("=================================== "+i++);
System.out.println("InteractionFragmentEditPart.updateCoveredLifelines(), line : "
...
//*/
				...
				if(!coveredLifelines.contains(lifeline1)) {
					...
					if (!origRect.intersects(lifelineRect)) {
						coveredLifelinesToAdd.add(lifeline1);							
					}						
				}
			// 새 경계와 lifeline 경계가 교차되지 않고
			// 원래 covered에 있던 lifeline은 remove
			} else {
				coveredLifelinesToRemove.add(lifeline1);
			}
		}
		/* apex improved end */
